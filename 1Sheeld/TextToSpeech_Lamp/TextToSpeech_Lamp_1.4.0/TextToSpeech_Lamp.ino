  ...
  if (state == 1) {
    ...
    if (VoiceRecognition.isNewCommandReceived()) {
      ...
      if (!strcmp(onCommand, VoiceRecognition.getLastCommand()) && digitalRead(lamp) == LOW) {			// LED on
		...
		stateRead:
		if (state == 6) {
			TextToSpeech.say("Please tell me the digital pin number");
			state = 7;
			delay(2000);
		}
		if (VoiceRecognition.isNewCommandReceived()) {
        ...
        help();
		  ...
		  TextToSpeech.say("1Sheeld is redy to use!");
	  }
	  ...
	  focus:
		  if (state == 8) {
			  TextToSpeech.say("Please tell me a digital pin number to focus");
			  state = 9;
			  delay(3500);
		  }
		  if (VoiceRecognition.isNewCommandReceived()) {
			  ...
			  if (error == 0) {
				  enableFocus = 1;
				  pinFocus = pinNb;
				  TextToSpeech.say("Focus has been activated");
				  state = 1;
			  }
			  else if (error == 1){
				  focusOn = 0;
		...
		focusName:
		if (state == 10) {
			TextToSpeech.say("Please tell me a new focus name");
			state = 11;
			delay(3500);
		}
		if (VoiceRecognition.isNewCommandReceived()) {
			focusName = VoiceRecognition.getLastCommand();
			TextToSpeech.say("The new focus name has been added");
			delay(3000);
			state = 1;		
		}
	  }
		...
		focusNameOn = 0;
		TextToSpeech.say("The focus name has been deactivated");
		delay(3000);
	  }
      else {
...
void help()
{
	...
	delay(2500);
	TextToSpeech.say("Focus,");
	delay(2500);
	TextToSpeech.say("Focus OFF,");
	delay(2500);
	TextToSpeech.say("Focus name,");
	delay(2500);
	TextToSpeech.say("Focus name OFF,");
	delay(2500);
	TextToSpeech.say("State,");
	delay(2500);
	TextToSpeech.say("Status.");
	delay(2500);
...
void focus()																// if focus function is ON --> check pin condition
{
	...
	{
		TextToSpeech.say("High condition on the pin: ");
		delay(2000);
		if (focusNameOn == 0) {
...
void focusOff()
{
	focusOn = 0;
	enableFocus = 0;
	TextToSpeech.say("Focus has been deactivated");
	delay(3500);
}
